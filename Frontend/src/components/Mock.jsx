...
const BACKEND = "https://ai-mock-interviewer-x7bb.onrender.com";
...
export default function App() {
  ...
  // ================= Renders =================
  return (
    <div className="h-screen w-screen font-inter text-white overflow-hidden relative">
      {/* Background gradient */}
        ...
        >
          ...
          {step === 3 && (
            <motion.div initial={{ y: 200, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className={`${sectionContainer} flex flex-col flex-1 min-h-[85vh] h-full w-full`}>
              ...
              <div ref={chatContainerRef} className="flex-1 overflow-y-auto space-y-4 mb-6 pr-4 custom-scrollbar w-full">
                {chat.map((msg, idx) => (
                  <motion.div
                    key={idx}
              ...
              <div className="flex flex-col items-center mt-6">
                <p className="text-sm text-gray-400 mb-2 text-center">
                  For the most accurate analysis, please click "Evaluate" after the interview is complete.
                </p>
                <button
                  onClick={evaluateInterview}
                  disabled={loading}
                  className={`bg-gray-700 px-8 py-3 rounded-full font-bold flex items-center transition-all duration-300 ${
